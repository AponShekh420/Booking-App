...
export default function NearByCard({ nearby, className }: NearbyT) {
	return (
		<div className={`nc-CardCategory5 flex flex-col relative bg-white ${className} `}>
			...
			<div
				className={`xl:mt-[10px] mt-2 xxs:mt-3 flex items-center justify-between text-[9.68px] xxs:text-[13.68px] xs:text-[15.28px] md:text-[16.28px] lg:text-[17px] xl:text-[20px]`}
			>
				<h3 className="xl:leading-[24px] leading-3 text-left">Hanley Laser Clinic - London</h3>
			</div>
			<div className="flex flex-wrap gap-y-0 items-center gap-x-2 text-[9.68px] xxs:text-[13.68px] font-semibold xs:text-[15.68px] md:text-[16.68px] lg:text-[17px] mt-1 xxs:mt-2 xs:mt-4">
				<Link className="text-custom-red leading-3" href={'/'}>
					Open Today
				</Link>
				<span className='leading-4'>9:30 am–8:30 pm</span>
			</div>
		</div>
	)
}
